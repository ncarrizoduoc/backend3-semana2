...
### Package config
Contiene las clases de configuración de los Jobs, Readers y Writers:
...
- **`EstadoCuentaItemReader`**: Clase que configura la lectura de movimientos de cuenta desde la tabla `MOVIMIENTO_CUENTA` de la base de datos.
- **`EstadoCuentaItemWriter`**: Clase que configura el guardado de objetos `EstadoCuenta` en la tabla `ESTADO_CUENTA` de la base de datos.
- **`InteresItemReader`**: Clase que configura la lectura de datos de cuentas bancarias desde el archivo `intereses.csv`.
